...
import { G, font, useAnalysis } from "./SEOTools.jsx";
import { useContent } from "./content.jsx";
import { Head } from "vite-react-ssg";
import { TOOLS, SITE_URL, BRAND } from "./tools.config.jsx";

// A short live "teaser" per tool, computed from the current analysis,
// so the home page reacts the moment you paste content.
...
function Metric({ label, value, color }) {
  return (
    <div style={{ background: G.surface, border: `1px solid ${G.border}`, borderRadius: 10, padding: "14px 18px", flex: 1, minWidth: 130 }}>
      <div style={{ fontSize: 12, color: G.textSec, fontWeight: 500, marginBottom: 6 }}>{label}</div>
      <div style={{ fontSize: 24, fontWeight: 700, color, textTransform: "capitalize", lineHeight: 1 }}>{value}</div>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    ...
    <div style={{ fontFamily: font, maxWidth: 1100 }}>
      <h1 style={{ fontSize: 28, fontWeight: 700, color: G.text, margin: "4px 0 8px" }}>Free SEO Content Analysis Tools</h1>
      <p style={{ fontSize: 15, color: G.textSec, lineHeight: 1.7, maxWidth: 760, marginBottom: 24 }}>
        Paste your content in the box above for an instant overview, then open any tool for the full breakdown — keyword density, long-tail phrases, search intent, competitor gaps and more across {TOOLS.filter(t => !t.noindex).length} dimensions.
      </p>

      {/* Instant overview — appears as soon as content is pasted */}
      {a && (
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ background: G.surface, border: `1px solid ${G.border}`, borderRadius: 10, padding: "12px 16px" }}>
      ...
      {/* Tool grid — each card shows a live count and links to its own page */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(250px,1fr))", gap: 12 }}>
        {TOOLS.filter((t) => !t.noindex).map((t) => (
          <Link key={t.slug} to={`/${t.slug}`} style={{ textDecoration: "none", background: G.surface, border: `1px solid ${G.border}`, borderRadius: 12, padding: 18, boxShadow: G.shadowSm, display: "block" }}>
