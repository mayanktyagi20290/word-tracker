...
function Dashboard({ text, analysis, isMobile }) {
  ...
  const stats = [
    { l: "Words", v: a.totalWords.toLocaleString() },
    { l: "Characters", v: chars.toLocaleString() },
    { l: "Sentences", v: a.sentences.toLocaleString() },
    { l: "Paragraphs", v: a.paragraphs.toLocaleString() },
    { l: "Reading", v: `${readMin} min` },
    { l: "Speaking", v: `${speakMin} min` },
  ];
    ...
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 12 }}>
      ...
      <GCard style={{ display: "flex", alignItems: "center", gap: 16, flexShrink: 0, minWidth: isMobile ? "auto" : 290 }} padding={isMobile ? 14 : 16}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: G.textSec }}>Biggest win: <b style={{ color: G.text }}>{worst.label}</b></div>
        </div>
      </GCard>

      {/* Stat tiles */}
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: isMobile ? "repeat(3,1fr)" : "repeat(6,1fr)", gap: 8 }}>
        {stats.map(s => (
          <GCard key={s.l} padding="12px 8px" style={{ textAlign: "center" }}>
            <div style={{ fontSize: isMobile ? 16 : 18, fontWeight: 800, color: G.text, lineHeight: 1.1, whiteSpace: "nowrap" }}>{s.v}</div>
            <div style={{ fontSize: 10, color: G.textSec, fontWeight: 600, marginTop: 4, textTransform: "uppercase", letterSpacing: .3 }}>{s.l}</div>
          </GCard>
        ))}
      </div>
    </div>
...
export default function SEOTracker() {
  ...
  const analysis = useMemo(() => {
    ...
    return {tokens,clean,freqEntries,totalWords,uniqueWords,phrases,questions,entities,intent,readability,structure,topKeywords,overused,stuffing,clusters,lsi,sentences,paragraphs,seo};
  },[text,filterStop]);
  ...
  const tabIcons = {frequency:"📊",longtail:"🔗",questions:"❓",nlp:"🧠",optimization:"⚡",highlighter:"🖍️",competitor:"🆚",structure:"🏗",intent:"🎯",export:"💾"};
