...
// ── HTML → readable text (used for URL-fetch fallback) ───────────────────────────
...
function clientExtract(html, url){
  ...
  // keep SEO structure as markdown so the Structure tab still works on fetched pages
...
export default function SEOTracker() {
  ...
  const fetchFromUrl = async () => {
    ...
    try {
      let data = null;
      // 1) try our own serverless endpoint (clean, no CORS issues)
      try {
      ...
      if (!data.text || data.text.trim().length < 20) {
        throw new Error("Page mil gaya par readable text nahi mila. Manual content paste karke try karein.");
      }
      setText(data.text);
      setFetchedFrom(data.title ? `${data.title} — ${u}` : u);
    } catch (e) {
      setFetchError(e.message || "Fetch fail ho gaya. Site automated access block kar rahi ho sakti hai — content manually paste karein.");
    } finally {
      setFetching(false);
    }
  };
  ...
  return (
    <div style={{fontFamily:font,background:G.bg,minHeight:"100vh",display:"flex",flexDirection:"column",color:G.text}}>
      ...
      {/* ── Input toolbar: paste / URL toggle · fetch · live counts ── */}
      <div style={{background:G.surface,borderBottom:`1px solid ${G.border}`,padding:"8px 24px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",flexShrink:0}}>
        <div style={{display:"flex",gap:4,background:G.bg,borderRadius:999,padding:3,border:`1px solid ${G.border}`}}>
          {[{id:"paste",label:"📝 Paste content"},{id:"url",label:"🔗 Fetch from URL"}].map(m=>{
            const on=inputMode===m.id;
            return (
              <button key={m.id} onClick={()=>{setInputMode(m.id);setFetchError("");}}
                style={{padding:"5px 14px",borderRadius:999,border:"none",cursor:"pointer",fontFamily:font,fontSize:12.5,fontWeight:500,
                        background:on?G.blue:"transparent",color:on?"#fff":G.textSec,transition:"all .15s"}}>
                {m.label}
              </button>
            );
          })}
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,flex:1,minWidth:280,flexWrap:"wrap"}}>
            <div style={{position:"relative",flex:1,minWidth:220}}>
              <input
                ...
                style={{width:"100%",height:38,background:G.bg,border:`1px solid ${G.border}`,borderRadius:8,
                        padding:"0 14px",fontFamily:font,fontSize:13,color:G.text,outline:"none",boxSizing:"border-box"}}
            ...
            <button onClick={fetchFromUrl} disabled={fetching||!urlInput.trim()}
              style={{height:38,padding:"0 18px",borderRadius:8,border:"none",cursor:fetching||!urlInput.trim()?"default":"pointer",
                      background:fetching||!urlInput.trim()?G.border:G.blue,color:fetching||!urlInput.trim()?G.textHint:"#fff",
                      fontFamily:font,fontSize:13,fontWeight:500,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:6}}>
              {fetching ? "⏳ Fetching…" : "Fetch & analyze"}
            </button>
            {text && <button onClick={()=>{setText("");setFetchedFrom("");setUrlInput("");setFetchError("");}}
              style={{height:38,padding:"0 12px",borderRadius:8,border:`1px solid ${G.border}`,background:"#fff",color:G.textSec,cursor:"pointer",fontFamily:font,fontSize:12.5,whiteSpace:"nowrap"}}>Clear</button>}
          </div>
        )}

        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:14,fontSize:12.5,color:G.textSec,whiteSpace:"nowrap"}}>
      ...
      {/* fetch status row */}
      {(fetchError || (inputMode==="url" && fetchedFrom)) && (
        <div style={{padding:"6px 24px",fontSize:12,flexShrink:0,
                     background:fetchError?G.redLight:G.greenLight,
                     color:fetchError?G.red:G.green,borderBottom:`1px solid ${G.border}`}}>
          {fetchError ? `⚠️ ${fetchError}` : `✅ Loaded from: ${fetchedFrom}`}
        </div>
      )}
          ...
          {!text.trim() ? (
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"60vh",gap:16,textAlign:"center"}}>
              ...
              <div style={{fontSize:14,color:G.textSec,maxWidth:400}}>
                Type or paste your text in the search bar above — or switch to <b>🔗 Fetch from URL</b> to pull a live page automatically. Supports 50,000+ words with real-time SEO analysis across 9 dimensions.
              </div>
