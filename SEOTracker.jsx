  ...
  return (
    <div style={{fontFamily:font,background:G.bg,minHeight:"100vh",display:"flex",flexDirection:"column",color:G.text}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden",marginTop:8}}>
              ...
              return (
                ...
                >
                  <span style={{fontSize:18,lineHeight:1}}>{
                    {frequency:"📊",longtail:"🔗",questions:"❓",nlp:"🧠",optimization:"⚡",competitor:"🆚",structure:"🏗",intent:"🎯",export:"💾"}[t.id]
                  }</span>
                  <span>{t.label}</span>
                </button>
              );
        ...
        {/* ── Content area ── */}
        <main style={{flex:1,overflowY:"auto",padding:"16px 24px 32px"}}>
          {!text.trim() ? (
            ...
            <TabRouter tab={tab} analysis={analysis} compText={compText} setCompText={setCompText} compAnalysis={compAnalysis} phraseLen={phraseLen} setPhraseLen={setPhraseLen} customN={customN} setCustomN={setCustomN} exportCSV={exportCSV} exportFullCSV={exportFullCSV} exportJSON={exportJSON} />
          )}
        </main>
      </div>
    </div>
